...
export default function Navbar() {
  ...
  // Render a stable skeleton on the server / first client render so
  // wallet-connection state differences don't cause hydration mismatch.
  if (!mounted) {
    return (
      <nav className="sticky top-0 z-50 border-b border-emerald-900/30 bg-[#0a0a0a]/90 backdrop-blur-md">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="flex h-16 items-center justify-between">
            ...
            <button
              disabled
              className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 px-3 py-2 text-sm font-medium text-emerald-300 opacity-60 cursor-not-allowed"
            >
              Connect Wallet
            </button>
          </div>
