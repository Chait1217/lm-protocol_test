...
export default function PolymarketLiveChart() {
  ...
  const intervalConfig =
    INTERVALS.find((i) => i.key === selectedInterval) || INTERVALS[1];

  const fetchHistory = useCallback(async () => {
    try {
      const res = await fetch(
        `/api/polymarket-history?interval=${selectedInterval}&fidelity=${intervalConfig.fidelity}`,
        { cache: "no-store" }
      );
    ...
    setLoading(false);
  }, [selectedInterval, intervalConfig.fidelity, data.length]);
  ...
  // Track canvas size on resize for crisp rendering
  useEffect(() => {
  ...
  }, [data, hover, selectedInterval, canvasSize]);
